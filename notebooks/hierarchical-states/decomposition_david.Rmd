...
ggsave("figs/david_jsd_time.pdf")
```
...
ggsave("figs/david_dist_distrib.pdf")
```
...
kNNdistplot(xform[, .(x, y)], mp) # identifies knee at y ~ 0.3 for eps value
xform[, dclust := dbscan(cbind(x, y), 0.3, mp)$cluster]
...
plot_paths_with_var("dclust", lo) + labs(color = "cluster") 
ggsave("figs/david_density_clusters.pdf")
```
```{r}
setkey(david.samples, sample)
setkey(xform, sample)
david.samples[xform[, .(sample, dclust)]] %>% 
  ggplot(aes(x = day, y = num_2_fac(dclust))) +
  geom_line(aes(group = subject, color = event)) +
  geom_point(aes(color = event), size = 0.5) +
  scale_color_brewer(palette = "Dark2") +
  ...
  labs(y = "cluster") +
  facet_wrap(~ subject, nrow = 2, scales = "free_y")
ggsave("figs/david_density_clusters_time.pdf")
```
### Transitions
```{r}
david.samples[xform[, .(sample, dclust)]] %>% 
...
```{r david-merged-tree,fig.width=7,fig.height=20,include=FALSE,eval=FALSE}
...
```{r david-event-trees,fig.height=9,fig.width=7,include=FALSE,eval=FALSE}
