...
Again, this method doesn't seem to give much extra information for this dataset.
...
```
```{r}
hot.div[, depth := factor(depth, levels = as.character(
  sort(as.numeric(unique(depth)))
))]
ggplot(hot.div, aes(x = distance)) +
  stat_ecdf(aes(color = depth))
...
hot.eigranks[, depth := factor(depth, levels = hot.depths)]
```
```{r}
ggplot(hot.eigranks, aes(x = rank, y = value)) +
