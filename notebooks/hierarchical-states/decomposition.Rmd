...
# metadata
events <- data.table(start = c(71, 80, 104, 151), end = c(122, 85, 113, 159),
                     subject = c("A", "A", "A", "B"),
                     event = c("travel", "diarrhea", "diarrhea", "Salmonella"))
...
david.div[day.delta == 1] %>% 
  ...
  geom_rect(data = events, mapping = aes(xmin = start, xmax = end, fill = event,
                                         ymin = 0.1, ymax = 1.05), 
            alpha = 0.5,
            inherit.aes = FALSE) +
  geom_point() +
...
# function to make graph from day-day distance data
DayDayGraph <- function(dt, metadata = NULL) {
  library(ggraph)
  library(igraph)
...
a.graph <- DayDayGraph(david.div["A"], events["A"])
# a.layout <- create_layout_tsne(a.graph$graph, a.graph$dm, 0)
# a.layout <- create_layout_nmds(a.graph$graph, a.graph$dm, trymax = 200)
a.layout <- create_layout(a.graph$graph, "mds", dist = a.graph$dm)
p.path.a <- ggraph(a.layout) +
  geom_edge_link(arrow = arrow(type = "closed",
                               length = unit(5, "points")),
                 edge_width = 0.2,
                 end_cap = square(length = 5, unit = "points")
                 ) +
  geom_node_point(aes(color = event)) +
  theme_graph(base_family = "Helvetica") #+
  # scale_color_distiller(palette = "YlOrRd")
b.graph <- DayDayGraph(david.div["B"], events["B"])
# b.layout <- create_layout_tsne(b.graph$graph, b.graph$dm, seed = 1)
# b.layout <- create_layout_nmds(b.graph$graph, b.graph$dm, trymax = 100)
b.layout <- create_layout(b.graph$graph, "mds", dist = b.graph$dm)
p.path.b <- ggraph(b.layout) +
  geom_edge_link(arrow = arrow(type = "closed",
                               length = unit(5, "points")),
                 edge_width = 0.2,
                 end_cap = square(length = 5, unit = "points")
  ...
  theme_graph(base_family = "Helvetica") #+
  # scale_color_distiller(palette = "YlOrRd")
plot_grid(p.path.a, p.path.b, nrow = 2,
          # rel_widths = c(3, 2),
          labels = "AUTO")
```
...
a.dendro <- Dist2Dendro(a.graph$dm)
a.dendro <- dendrapply(a.dendro, function(d) {
  if (is.leaf(d)) {
    day <- attr(d, "nodePar")$day
    if (day < 71) {
      attr(d, "nodePar") <- list(day = day, event = "US (pre)")
    } else if (day >= 71 & day <= 122) {
      if (day >= 80 & day <= 85) {
        attr(d, "nodePar") <- list(day = day, event = "travel + diarrhea 1")
      } else if ((day >= 104 & day <= 113)) {
        attr(d, "nodePar") <- list(day = day, event = "travel + diarrhea 2")
      } else {
        attr(d, "nodePar") <- list(day = day, event = "travel")
      }
    } else {
      attr(d, "nodePar") <- list(day = day, event = "US (post)")
    }
  }
  d
})
a.dendro <- tree_apply(a.dendro, function(node, children, depth, tree) {
  if (!is.leaf(node)) {
...
p1 <- ggraph(a.dendro, "dendrogram", circular = TRUE) +
  geom_edge_elbow(aes(color = node2.event)) +
  geom_node_point(aes(filter = leaf, color = day), size = 0.6) +
  scale_color_distiller(palette = "YlOrRd") +
  theme_graph(base_family = "Helvetica") +
  theme(aspect.ratio = 1)
b.dendro <- Dist2Dendro(b.graph$dm)
b.dendro <- dendrapply(b.dendro, function(d) {
  if (is.leaf(d)) {
    day <- attr(d, "nodePar")$day
    if (day < 151) {
      attr(d, "nodePar") <- list(day = day, event = "pre-Salmonella")
    } else if (day >= 151 & day <= 159) {
      attr(d, "nodePar") <- list(day = day, event = "Salmonella")
    } else {
      attr(d, "nodePar") <- list(day = day, event = "post-Salmonella")
    }
  }
  d
})
b.dendro <- tree_apply(b.dendro, function(node, children, depth, tree) {
  if (!is.leaf(node)) {
    events <- sapply(children, function(c) {
      attr(c, "nodePar")$event
    })
...
p2 <- ggraph(b.dendro, "dendrogram", circular = TRUE) +
  geom_edge_elbow(aes(color = node2.event)) +
  geom_node_point(aes(filter = leaf, color = day), size = 1) +
  scale_color_distiller(palette = "YlOrRd") +
  theme_graph(base_family = "Helvetica") +
  theme(aspect.ratio = 1)
...
gordon <- fread(paste0(data.dir, "cholera/gordon.otus"),
                col.names = c("sample", "otu", "count"))
gordon[, c("subject", "state", "sample") := tstrsplit(sample, "_")]
gordon[grep("d", sample) & state == "recovery", time.unit := "day"]
gordon[!grep("d", sample), time.unit := "hour"]
gordon[sample == "end", time.unit := "hour"]
setkey(gordon, time.unit)
gordon["hour", hour := as.numeric(sample)]
gordon["day", hour := 24 * as.numeric(sub("d", "", sample))]
# set end hour to last hour + 1, days to end hour + 24 * days
gordon[, hour := {
  maxhr <- max(.SD["hour"][sample != "end"]$hour)
  endhr <- maxhr + 1
  hour[sample == "end"] <- endhr
