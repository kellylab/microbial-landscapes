...
fits <- lapply(bats.depths, function(de) {
  d <- bats.div[depth == de]
  dm <- MakeDistMatrix(d, "id.i", "id.j")
  CoordCMDS(dm)
})
...
```
```{r bats-state-plots,fig.width=7,fig.height=9}
...
```{r bats-mds-eigen}
ggplot(fit$eigrank, aes(x = rank, y = value)) +
  geom_point()
```
...
p2 <- ggraph(lo) +
  geom_edge_link(arrow = arrow(length = unit(3, "points"), type = "closed"),
                 end_cap = square(length = 3, unit = "points"),
                 edge_width = 0.2) +
  geom_node_point(aes(color = state), size = nsize) +
  facet_nodes(~ subject) +
  theme_graph(base_family = "Helvetica") +
  theme(aspect.ratio = 1)
