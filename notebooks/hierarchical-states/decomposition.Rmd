...
dm <- as.matrix(dm[, -1])
rownames(dm) <- rn
ts.layout <- create_layout_tsne(graph, dm, 1)
...
a.graph <- DayDayGraph(david.div["A"])
...
b.graph <- DayDayGraph(david.div["B"])
...
Paths:
```{r gordon-paths,fig.width=7,fig.height=10}
setkey(gordon.div, subject)
setkey(gordon, subject)
paths <- lapply(subjects, function(subj) {
  ...
  layout <- create_layout(graph, "mds", dist = dm)
  p <- ggraph(layout) +
    geom_edge_link(arrow = arrow(type = "closed", length = unit(2, "points")),
                   edge_width = 0.2, end_cap = square(length = 2, 
                                                      unit = "points")) +
    scale_color_distiller(palette = "YlOrRd", trans = "log10") +
    geom_node_point(aes(color = hour, shape = state)) +
    theme_graph(base_family = "Helvetica")
})
