...
```
```{r,fig.asp=1}
pl <- lapply(sites, function(si, graf, coord, mbreaks) {
  gf <- graf[[si]]
...
pl[[1]]
ggsave("figs/prochloro_bats_calmonth.pdf")
pl[[2]]
ggsave("figs/prochloro_hot_calmonth.pdf")
```
...
```
```{r,fig.asp=1}
pl <- lapply(sites, function(si, grafs, xform) {
  g <- grafs[[si]]
...
pl[[1]]
ggsave("figs/bats_louvain.pdf")
pl[[2]]
ggsave("figs/hot_louvain.pdf")
```
...
nrow = 2)
ggsave("figs/prochloro_state_prob.pdf")
```
...
nrow = 2)
ggsave("figs/prochloro_transition_prob.pdf")
```
