...
plot_grid(plotlist = lapply(sites, function(si, xform) {
  xf <- xform[si]
  ggplot(xf, aes(x = x, y = y, color = cluster)) +
    geom_point() +
    # hack bc bats-40m has 1 pt in cluster 2 causing contours to fail for 
    # whole subplot
    geom_density_2d(data = xf[cluster == "1"]) +
    geom_density_2d(data = xf[cluster == "2"]) + 
    labs(color = "cluster") +
    theme_graph(base_family = "Helvetica") +
    theme(aspect.ratio = 1) +
    labs(title = si) +
