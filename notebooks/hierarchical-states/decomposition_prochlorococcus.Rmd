...
library(ggquiver)
# TODO make this work
plot_grid(plotlist = lapply(sites, function(si, xform, jsds) {
  ...
  # normalization
  merged[, m := sqrt(dx ^ 2 + dy ^ 2)]
  merged[, c("dx", "dy") := lapply(list(dx, dy), function(v, m) v / m, m = m)]
  # /normalization
  ggplot(merged, aes(x = x.i, y = y.i)) +
    geom_quiver(aes(u = dx, v = dy)) +
    theme_graph(base_family = "Helvetica") +
    theme(aspect.ratio = 1) +
    labs(title = si)
}, xform = xform, jsds = jsds[delta == 1 & depth.i == depth.j]),
nrow = 2)
```
...
plot_grid(plotlist = lapply(sites, function(si, grafs, xform) {
  ...
  ggraph(g, "manual", node.positions = coor) +
    geom_edge_link(aes(color = num2fac(node2.cluster)), 
