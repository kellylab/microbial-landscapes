...
plot_grid(plotlist = lapply(c("bats", "hot"), function(si, jsds) {
  ...
  sd[, c("month.i", "month.j") := lapply(list(month.i, month.j), num2fac)]
...
out = mds.out) %>% 
  rbindlist(idcol = "site")
prochlorococcus[, sample := paste(month, depth, sep = "_")]
xform <- merge(xform, unique(prochlorococcus[, .(site, 
                                        sample, 
                                        month, 
                                        year, 
                                        cal.month, 
...
samples <- merge(samples, modules, by = c("site", "sample")) 
...
```{r,fig.asp=1.2}
setkey(samples, site)
plot_grid(plotlist = lapply(sites, function(si, samples) {
  ggplot(samples[si], aes(x = cal.month, y = as.character(cluster))) +
    geom_line(aes(group = year, color = num2fac(year))) +
    scale_x_continuous(breaks = seq(12)) +
    labs(color = "year", y = "cluster") +
    # theme_minimal() +
...
ggplot(jsds[delta > 0 & depth.i == depth.j], 
       aes(x = num2fac(depth.i), y = cal.month.i)) +
  geom_violin(aes(color = transition)) +
  coord_flip() +
