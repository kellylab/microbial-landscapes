...
```{r}
ts <- prochlorococcus[, .(abundance = mean(abundance)), by = .(site,
                                                               depth,
                                                               ecotype,
                                                               month,
                                                               year,
...
```{r}
ts[, rel.abund := abundance / sum(abundance),
   by = .(site, depth, month)]
ts[, log.rel.abund := log10(rel.abund)]
ggplot(ts, aes(x = month, y = log.rel.abund)) +
...
## Spectral analysis
```{r}
ts <- dcast(ts, site + depth + month ~ ecotype, value.var = "rel.abund")
```
```{r}
library(imputeTS)
spectra <- ts[, {
  m <- as.matrix(.SD) # TODO get rid of month col
  m <- m[order(month),]
  m[is.nan(m)] <- NA
  m <- as.ts(m)
  m <- na.interpolation(m)
  sp <- stats::spectrum(m, plot = FALSE)
  ll <- lapply(seq_len(ncol(sp$spec)), function(i) sp$spec[, i])
  names(ll) <- sp$snames
  ll$freq <- sp$freq
  ll
  }, 
  by = .(site, depth)]
spectra <- dcast(spectra, id.vars = c("site", "depth", "freq"))
```
...
```{r,fig.asp=1.4,include=FALSE,eval=FALSE}
...
```
```{r,fig.asp=1.4}
plot_grid(plotlist = mapply(function(x, y) {
...
## Modularization
```{r}
jsd_2_clusts <- function(i, j, weight) {
  g <- graph_from_data_frame(data.frame(i, j, weight = weight),
                             directed = FALSE,
                             vertices = unique(i))
...
bats.div[, c("cluster.i", "cluster.j") := jsd_2_clusts(id.i, id.j, 1 - jsd),
         by = depth]
hot.div[, c("cluster.i", "cluster.j") := jsd_2_clusts(id.i, id.j, 1 - jsd),
         by = depth]
```
